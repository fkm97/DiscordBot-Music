...
COOKIES_TXT_PATH = Path(__file__).with_name("cookies.txt")

def export_firefox_cookies_to_txt(outfile: Path = COOKIES_TXT_PATH):
    """
    Export YouTube/Google cookies from the local Firefox profile to a Netscape cookies.txt file.
    Run this on the EC2 box where Firefox is logged in.
    """
    if browser_cookie3 is None:
        raise RuntimeError("browser-cookie3 is not installed")

    # Load all Firefox cookies (default profile). If you have multiple profiles, you can pass a path.
    cj = browser_cookie3.firefox()  # uses ~/snap/firefox/... or ~/.mozilla/firefox/... automatically

    allow = (".youtube.com", "youtube.com", ".google.com", "google.com")
    lines = ["# Netscape HTTP Cookie File\n"]
    now = int(time.time())

    for c in cj:
        if not any(domain in c.domain for domain in allow):
            continue
        domain = c.domain
        include_sub = "TRUE" if domain.startswith(".") else "FALSE"
        path = c.path or "/"
        secure = "TRUE" if getattr(c, "secure", False) else "FALSE"
        expires = int(getattr(c, "expires", 0) or (now + 90*86400))  # 90-day default if missing
        name = c.name
        value = c.value
...
async def periodically_refresh_cookies(interval_seconds: int = 6*3600):
    """Refresh cookies.txt every few hours."""
    while True:
        try:
            path = export_firefox_cookies_to_txt()
            print(f"[cookies] refreshed -> {path}")
        except Exception as e:
            print(f"[cookies] refresh failed: {e}")
...
ytdl_format_options = {
    ...
    'cookiefile': str(COOKIES_TXT_PATH),   # <-- auto-regenerated by the task above
    # remove any 'http_headers': {'Cookie': ...}
}
...
async def on_ready():
    ...
    async def on_ready():
        ...
        # Create/refresh cookies.txt from the local Firefox profile
        try:
            export_firefox_cookies_to_txt()
        except Exception as e:
            print(f"[cookies] initial export failed: {e}")

        # Background refresher every 6 hours
        bot.loop.create_task(periodically_refresh_cookies(6*3600))
